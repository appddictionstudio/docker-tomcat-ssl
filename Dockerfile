...
RUN curl -O https://apache.osuosl.org/tomcat/tomcat-9/v9.0.37/bin/apache-tomcat-9.0.37.tar.gz
# RUN curl -O http://apache.spinellicreations.com/tomcat/tomcat-9/v9.0.22/bin/apache-tomcat-9.0.22.tar.gz
...
RUN echo $"<?xml version=\"1.0\" encoding=\"UTF-8\"?><tomcat-users xmlns=\"http://tomcat.apache.org/xml\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:schemaLocation=\"http://tomcat.apache.org/xml tomcat-users.xsd\" version=\"1.0\"><role rolename=\"manager-gui\"/><role rolename=\"admin-gui\"/><user username=\"tomcat\" password=\"World2k!\" roles=\"admin-gui,manager-gui\"/></tomcat-users>" > /opt/tomcat/conf/tomcat-users.xml

COPY server.xml /opt/tomcat/conf/server.xml
COPY setenv.sh /opt/tomcat/bin
COPY web.xml /opt/tomcat/webapps/manager/WEB-INF/web.xml
